...
static RWLock *g_rwl, *g_rwl1, *g_rwl2;
...
static void *agente_fun(void *ptr) {
  ...
  if (pa->rwl==g_rwl1) {
    ...
    if (pa->tipo==LEC) {
      if (escribiendo)
        fatalError(desc, "%s no cumple exclusion mutua\n", nombre);
      lectores++;
    }
    else { // pa->tipo==ESCR
...
void termina(Agente *pa, int ref) {
  ...
  if (dif<0)
    dif= -dif;
  if (dif>TAM_PASO/2)
    fatalError("termina", "Tiempo de termino %d de %s es incorrecto\n",
                t, nombre);
}
...
#   define MOD      50
  ...
  destroyRWLock(g_rwl1);
  destroyRWLock(g_rwl2);
  return 0;
}
